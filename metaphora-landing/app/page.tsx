...
const FloatingWords = () => {
  ...
  useEffect(() => {
    ...
    setRandoms({
      initialX: floatingWords.map(() => Math.random() * window.innerWidth),
      animateX: floatingWords.map(() => Math.random() * window.innerWidth),
      rotate: floatingWords.map(() => Math.random() * 20 - 10),
      duration: floatingWords.map(() => 12 + Math.random() * 8),
    });
  }, []);
    ...
    <div className="fixed inset-0 pointer-events-none z-10 overflow-hidden">
      {floatingWords.map((word, index) => (
        <motion.div
          ...
          initial={{
            x: randoms.initialX[index],
            y: (typeof window !== "undefined" ? window.innerHeight : 800) + 50,
            opacity: 0,
          }}
          animate={{
            x: randoms.animateX[index],
            y: -50,
            opacity: [0, 0.7, 0.7, 0],
            rotate: [0, randoms.rotate[index]],
          }}
          transition={{
            duration: randoms.duration[index],
            delay: index * 1.5,
...
const FloatingElements = () => {
  const [randoms, setRandoms] = useState<{
    ...
    featherX: number[];
    featherY: number[];
    featherDuration: number[];
  } | null>(null);

  useEffect(() => {
    if (typeof window === "undefined") return;
    setRandoms({
      leafY: Array.from({ length: 6 }, () => Math.random() * window.innerHeight),
      leafY2: Array.from({ length: 6 }, (_, i) => Math.random() * window.innerHeight + Math.sin(i) * 100),
      leafDuration: Array.from({ length: 6 }, () => 20 + Math.random() * 15),
      featherX: Array.from({ length: 4 }, () => (window.innerWidth) + 100),
      featherY: Array.from({ length: 4 }, () => Math.random() * window.innerHeight),
      featherDuration: Array.from({ length: 4 }, () => 25 + Math.random() * 10),
    });
  }, []);
    ...
    <div className="fixed inset-0 pointer-events-none z-10 overflow-hidden">
      {[...Array(6)].map((_, i) => (
        <motion.div
          ...
          transition={{
            duration: randoms.leafDuration[i],
            delay: i * 3,
            repeat: Number.POSITIVE_INFINITY,
            ease: "linear",
          }}
